...
\begin{verbatim}
Prim(Graph G):
    V <- v_0                            // arbitrary starting point
    U = G \ V
    MST = {}                            // initialize to empty set
    while V != G.V:                     // while not all vertices are in V
        e = (u, v) with minimum edge weight where u in U and v in V
        MST.add(e)
        U.remove(u)
        V.add(u)
    return MST
